...
def add_contact(contacts):
    ...
    contacts[name] = {"id": id, "phone": phone, "email": email}
...
# Display all contacts in a table format with column separation
def display_all_contacts(contacts):
    if not contacts:
        print("No contacts found.")
    else:
        # Print table headers
        print(f"{'ID':<15} | {'Name':<30} | {'Phone':<15} | {'Email':<30}")
        print("-" * 90)  # Print a separator line

        # Print each contact in table format
        for name, details in contacts.items():
            print(f"{details['id']:<15} | {name:<30} | {details['phone']:<15} | {details['email']:<30}")
...
def display_menu():
    ...
    print("A Add Contact")
    print("B Search Contact")
    print("C Update Contact")
    print("D Delete Contact")
    print("E Display All Contacts")
    print("F Exit")
